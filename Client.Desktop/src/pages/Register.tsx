...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const requiredFields = [
      ...
      { key: 'confirmPassword', name: 'Confirmar Contraseña' }
    ];
    ...
    if (data.password !== data.confirmPassword) {
      setAlertInfo({ type: 'error', message: 'Las contraseñas no coinciden' });
      return;
    }

    try {
      const res = await fetch('http://localhost:5219/api/auth/register-public', {
        method: 'POST',
      ...
      if (res.ok) {
        setAlertInfo({ type: 'success', message: '¡Registro Exitoso! Redirigiendo al Login...' });
        setTimeout(() => navigate('/login'), 2000);
      } else {
        const errorData = await res.json();
        
        // Si el backend responde que el correo ya existe
