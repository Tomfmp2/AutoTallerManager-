...
import { LogOut, Wrench, Settings, Car } from 'lucide-react';
import { useEffect } from 'react';
import LanguageSwitcher from '../components/LanguageSwitcher';
import NotificationDropdown from '../components/NotificationDropdown';

const navItems = [
  { to: '/mecanico/dashboard', icon: <Wrench className="w-5 h-5" />, label: 'Dashboard' },
  { to: '/mecanico/config', icon: <Settings className="w-5 h-5" />, label: 'Configuración' }
];

export default function MechanicLayout() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const email = localStorage.getItem('userEmail') || 'Mecánico';
  const displayName = email.includes('@') ? email.split('@')[0] : email;

  useEffect(() => {
    const role = localStorage.getItem('userRole');
    if (role !== 'Mecanico' && role !== 'Mecánico' && role !== 'Admin' && role !== 'Administrador') {
      navigate('/login');
    }
  }, [navigate]);
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex font-sans text-gray-800">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-8 shrink-0 shadow-sm z-10">
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Portal Mecánico</h2>
            <p className="text-sm text-gray-500">Hola de nuevo, experto {displayName}</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              className="w-10 h-10 rounded-full bg-gradient-to-tr from-orange-400 to-red-500 flex items-center justify-center text-white font-bold shadow-md"
            >
