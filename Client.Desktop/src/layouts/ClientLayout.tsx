...
import { LayoutDashboard, Car, CalendarClock, Settings, LogOut } from 'lucide-react';
import LanguageSwitcher from '../components/LanguageSwitcher';
import NotificationDropdown from '../components/NotificationDropdown';

const navItems = [
  { to: '/cliente/dashboard',     icon: <LayoutDashboard className="w-5 h-5" />, label: 'Resumen' },
  { to: '/cliente/vehiculos',     icon: <Car className="w-5 h-5" />,             label: 'Mis Vehículos' },
  { to: '/cliente/historial',     icon: <CalendarClock className="w-5 h-5" />,   label: 'Historial y Revisiones' },
  { to: '/cliente/configuracion', icon: <Settings className="w-5 h-5" />,        label: 'Configuración' },
];

export default function ClientLayout() {
  const navigate = useNavigate();
  const email = localStorage.getItem('userEmail') || 'Cliente';
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-20 bg-white/50 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500">Bienvenido de nuevo, {displayName}</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              ...
              className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold shadow-md cursor-pointer hover:scale-105 transition-transform"
            >
